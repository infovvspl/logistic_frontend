...
export default function BillForm({
  ...
  const uniqueFromPlaces = useMemo(
    () =>
      Array.from(
        new Set(
          challans
            .map((t) => challanMeta[String(t.id)]?.fromPlace)
            .filter(Boolean),
        ),
      ).sort(),
    [challans, challanMeta],
  );
  const uniqueToPlaces = useMemo(
    () =>
      Array.from(
        new Set(
          challans
            .map((t) => challanMeta[String(t.id)]?.toPlace)
            .filter(Boolean),
        ),
      ).sort(),
    [challans, challanMeta],
  ...
  const uniqueCustomers = useMemo(
    () =>
      Array.from(
        new Set(
          challans
            .map((t) => challanMeta[String(t.id)]?.customerName)
            .filter(Boolean),
        ),
      ).sort(),
    [challans, challanMeta],
  ...
  const validate = () => {
    ...
    if (!isEdit && selectedIds.length === 0)
      e.challan_id = "Select at least one challan";
    return e;
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (isEdit) {
      ...
      // console.log("Submitted:", { bill_no: form.bill_no, challan_ids: selectedIds })
    }
  };
    ...
    <form onSubmit={handleSubmit} className="w-full space-y-4">
      ...
      {!isEdit && selectedIds.length > 1 && form.bill_no && (
        <p className="text-[11px] text-amber-600 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2 -mt-2">
          ...
          <strong>
            {selectedIds.map((_, i) => `${form.bill_no}-${i + 1}`).join(", ")}
          </strong>
        </p>
      )}
        ...
        <div className="space-y-1.5">
          ...
          {selectedChallan ? (
            <div className="flex items-center justify-between rounded-lg border border-blue-100 bg-blue-50 px-3 py-2.5 text-sm text-blue-700">
              <div className="min-w-0">
                ...
                <div className="text-xs text-blue-500">
                  ...
                  {challanMeta[String(selectedChallan.id)]?.route
                    ? ` · ${challanMeta[String(selectedChallan.id)].route}`
                    : ""}
                </div>
              </div>
            ...
            <ChallanPicker
              ...
              uniqueFromPlaces={uniqueFromPlaces}
              uniqueToPlaces={uniqueToPlaces}
            ...
            <div className="flex flex-wrap gap-1.5">
              {selectedIds.map((id) => {
                ...
                return (
                  <span
                    key={id}
                    className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-50 border border-blue-100 text-xs font-semibold text-blue-700"
                  >
                    {c?.challan_no ?? id}
                    <button
                      type="button"
                      onClick={() => toggleChallan(id)}
              ...
              filteredChallans.map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {isSelected && (
                        <FiCheck size={10} className="text-white stroke-[3]" />
                      )}
                    </div>
                    <div className="min-w-0">
      ...
      <div className="flex justify-end pt-3 border-t border-zinc-100">
        {serverError && (
          <p className="flex-1 text-sm text-rose-600 font-medium self-center">
            ⚠ {serverError}
          </p>
        )}
        <Button
        ...
        >
          {isEdit
            ? "Update Bill"
            : `Create Bill${selectedIds.length > 1 ? `s (${selectedIds.length})` : ""}`}
        </Button>
      </div>
    </form>
  );
